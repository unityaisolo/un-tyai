...
namespace UnityAI
{
 ...
 private VisualElement _worldAfter;

 /// <summary>
 /// "Kurulumdan sonra" bölümünü sahnede Nova arazisi varsa gösterir.
 /// WorldPrep de aynı ada bakıyor (NovaTerra/NovaCity/NovaTown) — tek kaynak olsun diye
 /// burada da aynı önekler kullanılıyor.
 /// </summary>
 private void SyncWorldAfter()
 {
     if (_worldAfter == null) return;
     bool has = false;
     try
     {
         foreach (var go in UnityEngine.SceneManagement.SceneManager.GetActiveScene().GetRootGameObjects())
             if (go != null && (go.name.StartsWith("NovaTerra") || go.name.StartsWith("NovaCity") || go.name.StartsWith("NovaTown")))
             { has = true; break; }
     }
     catch { }
     _worldAfter.style.display = has ? DisplayStyle.Flex : DisplayStyle.None;
 }
 ...
 // Nehir kıvrımı yalnızca Irmak açıkken anlamlı. Eskiden hep görünüyordu ve
 // "Orman seçtim ama nehir ayarı duruyor" karışıklığına yol açıyordu.
 if (_worldRiverCurve != null && _wRiver != null)
 {
     Action syncRiver = () =>
         _worldRiverCurve.style.display = _wRiver.value ? DisplayStyle.Flex : DisplayStyle.None;
     _wRiver.RegisterValueChangedCallback(_ => syncRiver());
     syncRiver();
 }

 // "Kurulumdan sonra" bölümü: sahnede Nova arazisi yokken bu butonların hiçbir
 // işlevi yok; ana ekranı kalabalıklaştırmasınlar diye kurulana kadar gizli.
 _worldAfter = root.Q<VisualElement>("world_after");
 // Kurulum asenkron bitiyor; sahne değiştikçe yeniden bak (undo/sahne açma da kapsanır).
 EditorApplication.hierarchyChanged += SyncWorldAfter;
 SyncWorldAfter();
